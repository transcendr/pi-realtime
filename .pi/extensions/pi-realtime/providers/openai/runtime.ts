...
import { interactionMode } from "../../domain/interaction-modes";
...
function createOpenAIWebRTCMediaRuntime(deps: { debugTraces: DebugTraceRegistry; webrtcHelper: WebRTCHelperServer }): ProviderMediaRuntime {
	return {
		async start(input) {
			...
			const mode = interactionMode(input.session.interactionMode);
			const systemPrompt = mode.systemPrompt(input.surface);
			const adapter = createOpenAIWebRTCBridgeAdapter(input.session.providerSessionId, deps.webrtcHelper, () => createOpenAIWebRTCClientSecret({ model: input.session.model, instructions: systemPrompt, interaction: mode.providerInteraction }), trace);
			input.setAdapter(adapter);
			await adapter.connect({ providerSessionId: input.session.providerSessionId, provider: "openai", model: input.session.model, personaId: input.session.personaId, systemPrompt, toolSurface: input.surface, initialContext: input.packets[0], capabilities: { preferPassiveContext: false, preferSemanticVad: true }, interaction: mode.providerInteraction }, input.sink);
