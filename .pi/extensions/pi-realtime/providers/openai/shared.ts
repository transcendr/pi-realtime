...
function toolParameters(name: VoiceToolName): unknown {
	if (name === "request") return { type: "object", additionalProperties: false, properties: { request: { type: "string", description: "Concise system request capturing the user's intent, constraints, urgency, and relevant context." }, deliveryHint: { type: "string", enum: ["work", "progress"], description: "Use progress when the user is asking for current status/progress during an active task; those requests should steer the active turn. Use work for new work or ordinary questions." }, urgency: { type: "string", enum: ["normal", "interrupt"] }, userUtteranceSummary: { type: "string" }, citedCitationIds: { type: "array", items: { type: "string" } } }, required: ["request"] };
